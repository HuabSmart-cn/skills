...
async function processFile(p, info, buckets) {
  ...
  for await (const line of rl) {
    ...
    if (e.type === 'assistant') {
      ...
      if (
        !prev ||
        (usage.output_tokens || 0) >= ***REDACTED***
      ) {
  ...
  for (const [key, { usage, ts, skill, prompt }] of fileApiCalls) {
    ...
    if (prompt) {
      ...
      if (r) {
        ...
        r.outputTokens += ***REDACTED***
      }
    }

    // subagent token accounting on parent buckets
    if (info.kind === 'subagent') {
      overall.subagentTokens += ***REDACTED***
      project.subagentTokens += ***REDACTED***
      if (subagent) subagent.subagentTokens += ***REDACTED***
    }
...
... [Content truncated, total 28,201 chars] ...