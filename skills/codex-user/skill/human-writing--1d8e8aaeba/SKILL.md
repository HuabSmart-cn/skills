...
仅当当前任务已激活 `hongfenghua-ai-oral` 时，本分支优先于本文件及 references 中的通用修辞禁令；其他写作继续使用原规则。用户当前明确禁令始终优先。

保留有解释作用的对照、类比、模拟问答、自然口语、必要排比及标点；检查是否虚设误解、空转重复或遮蔽事实，不按句式、次数和符号清零。明确的假设场景可帮助讲解，但不能伪装为真实事件、用户亲历或产品能力证据，也不能用于重复扩写凑字数。

此分支以人工语境审稿收尾，不运行通用 `check_prose.py` 作为验收门槛。该脚本没有个人风格模式；若已运行，其修辞失败只供定位，不要求改到退出码为零，不宣称脚本通过。事实核验、材料充分性、用户指定修改范围与去除赘余继续执行。不要让最后一遍润色改变作者立场或磨掉已有的语气。
...
只服务当前作品。不自行建立长期作者画像，不生成个人规则库，不创建新的个人写作 Skill。用户明确要求使用已存在、具名的个人风格 Skill 时，可按该 Skill 的文体规则完成当前作品；本 Skill 继续负责事实边界、材料要求和自然中文，不把一次作品反馈扩张为新的长期规则。
