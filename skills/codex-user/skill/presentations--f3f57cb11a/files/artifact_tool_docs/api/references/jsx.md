...
- Use raw pixel numbers for fixed sizes: `width={420}`, `gap={24}`,
  `padding={{ x: 48, y: 40 }}`.
- Use Tailwind-like tokens for common styling: ***REDACTED***
...
| Visual tokens | `className=***REDACTED***
...
<column width="fill" height="hug" gap={8}>
  ...
  <paragraph bulletCharacter="•" marginLeft={18 * 12700} indent={-10 * 12700}>
    Bulleted paragraph with native list metadata.
  </paragraph>
</column>
```

Paragraph indentation uses EMU, not pixels; the example converts points with
12,700 EMU per point. Paragraph spacing uses hundredths of a point. See
[native bullets](../../../references/native_bullets.md).
...
- `tests/presentation/tailwind-tokens.sanity.spec.ts`: ***REDACTED***
