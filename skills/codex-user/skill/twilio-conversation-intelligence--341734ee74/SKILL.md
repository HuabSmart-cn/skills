...
| `GET` | `/v3/InsightsDomains/Conversations/Query?pageToken=***REDACTED***
...
async function* queryAll(queryBody) {
  ...
  while (nextToken) {
    const page = await fetch(
      `${INSIGHTS_BASE}/v3/InsightsDomains/Conversations/Query?pageToken=***REDACTED***
      { headers: getAuthHeaders() }
    ).then(r => r.json());
    yield* page.items;
    nextToken = ***REDACTED***
