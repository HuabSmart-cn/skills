...
`<account-index>` must be decimal digits. `<mailbox-view>` must be one of `all`, `inbox`, `sent`, `starred`, `snoozed`, `drafts`, `trash`, `spam`, or `important`. The fragment must contain exactly the mailbox view and one non-empty token. Ignore a query suffix attached to the token, such as `?attachment_id=***REDACTED***
...
2. Call `read_email_thread` with the token as a message ID, using the tool's default `id_type=***REDACTED***
3. Only when that exact lookup reports that the ID is invalid or not found, retry once with the same token and `id_type=***REDACTED***
