...
1. Find server runtime and outbound API conventions.
   ...
   - Search for existing config and secret retrieval patterns before defaulting to env vars: ***REDACTED***
...
7. Add web attribution context for CAPI when available.
   ...
   - If the server cannot read the cookie but an existing browser-to-server conversion request exists, add a minimal optional context object such as `openaiAds: ***REDACTED***
