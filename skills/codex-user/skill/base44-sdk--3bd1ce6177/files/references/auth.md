...
interface AuthModule {
  ...
  // Password Management
  resetPasswordRequest(email: ***REDACTED***
  resetPassword(params: ***REDACTED***
  changePassword(params: ***REDACTED***
}
```
...
try {
  const response = await base44.auth.loginViaEmailPassword(
    "user@example.com",
    "password123",
    turnstileToken  // optional: ***REDACTED***
  );
...
try {
  ...
  console.error('Failed to set token:***REDACTED***
}
```
...
try {
  ...
  console.error('Password reset request failed:***REDACTED***
  if (error.status === 429) {
...
try {
  ...
  console.error('Password reset failed:***REDACTED***
  if (error.status === 400) {
...
try {
  ...
  console.error('Password change failed:***REDACTED***
  if (error.status === 401) {
