...
| **Authorization Code** | User-facing web apps | User consent flow | 1 hour (refresh: ***REDACTED***
| **Authorization Code + PKCE** | SPAs, mobile apps | User consent flow | 1 hour (refresh: ***REDACTED***
...
class ZoomS2SAuth {
  constructor(accountId, clientId, clientSecret) {
    ...
    this.clientSecret = ***REDACTED***
    this.token = ***REDACTED***
    this.tokenExpiry = ***REDACTED***
  }
  ...
  async getAccessToken() {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Token error: ***REDACTED***
    }
