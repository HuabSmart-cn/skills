...
description: >
  Set up and manage Twilio authentication credentials: ***REDACTED***
...
token.add_grant(VoiceGrant(outgoing_application_sid=***REDACTED***
print(token.to_jwt())
```
...
token.addGrant(new VoiceGrant({ outgoingApplicationSid: ***REDACTED***
...
- **PKCV is an advanced feature for compliance-heavy industries** — Public Key Client Validation adds client-certificate-style auth. Incompatible with Flex, Studio, and TaskRouter. Once enforcement is enabled, Auth Token authentication is disabled (one-way door). See [PKCV docs](https:***REDACTED***
