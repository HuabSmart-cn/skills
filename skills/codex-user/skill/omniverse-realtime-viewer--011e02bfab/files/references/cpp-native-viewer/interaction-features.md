...
static std::string resolvePrimPathId(ovrtx_renderer_t* renderer, ovx_primpath_t pathId)
{
    ...
    ovx_token_t* tokensPerPath[] = ***REDACTED***
    size_t tokenCounts[] = ***REDACTED***
    size_t pathsProcessed = 0;

    ovx_api_result_t tokenResult = ***REDACTED***
        &dictionary,
        &pathId,
        1,
        tokenBuffer.data(),
        ...
        tokenCounts,
        &pathsProcessed);
    if (tokenResult.status != ***REDACTED***
        return {};
    }
    ...
    std::string path;
    for (ovx_string_t token : ***REDACTED***
