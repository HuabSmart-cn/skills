...
**Why `"$HF_TOKEN"` fails with `HfApi().run_uv_job()`:***REDACTED***
...
try:
    ...
    print(f"✅ Token valid for user: ***REDACTED***
except Exception as e:
    raise ValueError(f"Token validation failed: ***REDACTED***
```
...
- [ ] Job includes `secrets=***REDACTED***
- [ ] Script checks for token: ***REDACTED***
