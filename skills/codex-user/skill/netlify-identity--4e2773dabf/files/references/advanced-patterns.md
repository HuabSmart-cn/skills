...
async function handleForgotPassword(email: ***REDACTED***
  try {
...
async function handlePasswordReset(newPassword: ***REDACTED***
  try {
    await updateUser({ password: ***REDACTED***
...
async function handleAcceptInvite(token: ***REDACTED***
  try {
...
async function handleEmailChangeVerification(token: ***REDACTED***
  try {
