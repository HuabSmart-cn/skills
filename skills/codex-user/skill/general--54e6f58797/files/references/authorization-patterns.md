...
function requireScopes(requiredScopes) {
  return async (req, res, next) => {
    ...
    try {
      ...
      const tokenInfo = ***REDACTED***

      // Check if token has all required scopes
      const tokenScopes = ***REDACTED***
...
async function getTokenInfo(accessToken) {
  ...
  // For User OAuth tokens: ***REDACTED***
...
async function handleOAuthCallback(code) {
  ...
  const { access_token, refresh_token, scope, expires_in } = ***REDACTED***
  ...
  for (const op of operations) {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        throw new Error(`Authorization failed at step "${op.name}": ***REDACTED***
      }
      throw error;
    }
  }
