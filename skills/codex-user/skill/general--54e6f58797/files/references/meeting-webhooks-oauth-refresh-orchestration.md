...
export class TokenBroker {
  ...
  constructor(
    ...
    private clientSecret: ***REDACTED***
  ) {}

  async getToken(): ***REDACTED***
    const now = Date.now();
  ...
  invalidate() {
    this.state.accessToken = ***REDACTED***
  ...
  private async refresh(): Promise<string> {
    ...
    if (!res.ok) throw new Error(`token_refresh_failed:***REDACTED***
...
```ts
export async function createMeeting(tokenBroker: ***REDACTED***
...
export async function handleWebhook(req: Request, res: Response, secret: string, enqueue: (e: any) => Promise<void>) {
  if (req.body?.event === 'endpoint.url_validation') {
    const plainToken = ***REDACTED***
    const encryptedToken = ***REDACTED***
