...
const getToken = ***REDACTED***
  ...
  if (!token) {
    ...
    const { access_token, expires_in } = ***REDACTED***
...
```javascript
const tokenMiddleware = ***REDACTED***
  const userId = req.session.userId;

  // Get user's tokens from database
  let { access_token, refresh_token, token_expiry } = ***REDACTED***
...
const response = await refreshToken(old_refresh_token);
const { access_token, refresh_token } = ***REDACTED***
...
4. **Refresh token expires (lifetime varies):***REDACTED***
...
await db.query('UPDATE users SET token = ***REDACTED***
```
