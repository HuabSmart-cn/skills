...
const { ZOOM_ACCOUNT_ID, ZOOM_CLIENT_ID, ZOOM_CLIENT_SECRET } = ***REDACTED***
...
const getToken = ***REDACTED***
  try {
    ...
    throw new Error(`Token request failed: ***REDACTED***
  }
};

const setToken = ***REDACTED***
...
const { getToken, setToken } = ***REDACTED***

const tokenCheck = ***REDACTED***
  let token = ***REDACTED***

  // Redis returns null if key doesn't exist
  if (!token) {
    try {
      const { access_token, expires_in, error } = ***REDACTED***
