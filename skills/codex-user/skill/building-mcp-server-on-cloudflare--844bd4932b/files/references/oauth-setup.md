...
function generateCSRFProtection(): { token: string; setCookie: string } {
  ...
  const setCookie = ***REDACTED***
  return { token, setCookie };
}

// Validate token when user approves
function validateCSRFToken(formData: ***REDACTED***
  const tokenFromForm = ***REDACTED***
  const cookieHeader = ***REDACTED***
  const tokenFromCookie = ***REDACTED***
    .split(";")
    .find((c) => c.trim().startsWith("__Host-CSRF_TOKEN="))
    ?.split("=")[1];

  if (!tokenFromForm || !tokenFromCookie || tokenFromForm !=***REDACTED***
    throw new Error("CSRF token mismatch");
  }

  return {
    clearCookie: ***REDACTED***
  };
}
```
...
```typescript
function buildSecurityHeaders(setCookie: ***REDACTED***
  const cspDirectives = [
    "default-src 'none'",
...
async function bindStateToSession(stateToken: ***REDACTED***
...
- [MCP Authorization Spec](https:***REDACTED***
