...
const USAGE =
  "Usage: create-template-skill.mjs --reference-path <path> [--preview-path <path>] --display-name <name> --description <description> [--kind <kind>] [--gallery-kind <kind>] [--source-url <url>] [--mode update --skill-name <name>]";
const templateKinds = new Map([
  ...
  [
    "site",
    {
      ...
      workflow: `2. Load [@Sites](plugin://sites@openai-bundled) and copy the retained source directory into a new working directory.
        ...
        ? fs.cp(referencePath, path.join(stagedSkill, "assets", "source"), {
            filter: async (sourcePath) => {
              ...
              return (
                ...
                !["coverage", "dist", "out", "outputs", "private", "work"].includes(
...
function getTemplateSkillMarkdown({
  ...
  const triggerDescription = `Create ${template.output} using the ${displayName} template and its ${referenceDescription}. Use when the user selects this template, names ${displayName}, or explicitly invokes $${skillName}. ${description}`;
  return `---
name: ${skillName}
...
function getGoogleWorkspaceWorkflow(skillName, sourceArtifact) {
  return `2. Load the \`${skillName}\` skill from [@Google Drive](plugin://google-drive@openai-curated-remote) and treat \`sourceUrl\` as the native template reference.
3. Keep the source read-only. Copy the complete ${sourceArtifact} before editing, then follow the loaded skill's native reference/template workflow.
...
5. Adapt the copied native structure instead of rebuilding it through an Office file or replacing its visual system with generic defaults.
6. Verify the finished ${sourceArtifact} with the loaded skill's required structural and visual checks, then return its Google Drive link.`;
}
...
... [Content truncated, total 29,961 chars] ...