...
This single call handles both seaborn theme setup and CJK font configuration in the correct order. Do NOT manually set `plt.rcParams['font.sans-serif']` or call `sns.set_theme()` / `sns.set_style()` — `init_plot_env()` does both.
...
| Search extracted files | `scripts/search_extracted.sh` | `bash scripts/search_extracted.sh extracted "long query" [--regex]` |
