...
| `data.next_page_token` | 下一页 token；当 `has_more=***REDACTED***
...
1. 初始化待处理队列，放入起点目录：
   - 普通文件夹：`{folder_token:***REDACTED***
   - Drive 根目录：`{folder_token:***REDACTED***
2. 从队列取出一个目录，请求第一页。
...
while queue not empty:
  ...
  while true:
    ...
    if next is empty:
      retry_without_token += ***REDACTED***
      if retry_without_token >= ***REDACTED***
...
| 错误用法 | 问题 | 正确做法 |
|----------|------|----------|
| `lark-cli drive files list --folder-token <token>` | `files.list` 不提供 `--folder-token` flag | 使用 `--params '{"folder_token":***REDACTED***
