...
知识库链接（`/wiki/TOKEN`）不能直接当 `xml_presentation_id`。直接调用原生 API 前，先查询 wiki 节点，确认 `node.obj_type =***REDACTED***

```bash
lark-cli wiki spaces get_node --as user --params '{"token":***REDACTED***
```
...
| [`+media-upload`](references/lark-slides-media-upload.md) | 上传本地图片到指定演示文稿，返回 `file_token`（用作 `<img src=***REDACTED***
