...
> **按需裁剪**：`subtitle` / `text_tag_list` / color token 按实际诉求取舍，不强制全用。组件里用 `"text_size": ***REDACTED***
...
| `style.text_size` | — | 自定义字号 token，格式 `{"<名称>":***REDACTED***
| `style.color` | — | 自定义颜色 token，格式 `{"<名称>":***REDACTED***
