...
app.get("/api/check-login", async (req, res) => {
  ...
  if (apiKey || authToken) {
    ...
    // 脱敏显示
    if (apiKey) {
      response.envVars!.apiKey = ***REDACTED***
      response.apiKey = ***REDACTED***
    }
    if (authToken) {
      response.envVars!.authToken = ***REDACTED***
    }
    if (internetEnv) {
...
app.post("/api/save-env-config", (req, res) => {
  const { apiKey, authToken, internetEnv, baseUrl } = ***REDACTED***
  ...
  // 设置环境变量（仅在当前进程有效）
  if (apiKey) {
    process.env.CODEBUDDY_API_KEY = ***REDACTED***
    configuredVars.push('CODEBUDDY_API_KEY');
  }
  if (authToken) {
    process.env.CODEBUDDY_AUTH_TOKEN = ***REDACTED***
