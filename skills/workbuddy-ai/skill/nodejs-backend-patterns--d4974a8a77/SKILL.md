...
export class UserService {
  ...
  async createUser(userData: CreateUserDTO): Promise<User> {
    ...
    const { password, ...userWithoutPassword } = ***REDACTED***
...
export class AuthService {
  ...
  async refreshToken(refreshToken: ***REDACTED***
    try {
      const payload = jwt.verify(
        refreshToken,
  ...
  private generateToken(payload: ***REDACTED***
    return jwt.sign(payload, process.env.JWT_SECRET!, {
      expiresIn: '15m'
    });
  }

  private generateRefreshToken(payload: ***REDACTED***
