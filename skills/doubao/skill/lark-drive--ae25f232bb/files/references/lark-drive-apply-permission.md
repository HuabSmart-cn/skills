...
- **仅支持 `user` 身份**（使用 `user_access_token`）；shortcut 已在 `AuthTypes` 中强制限定为 `user`。
...
传入 `/wiki/<node_token>` 时，shortcut 会直接用 `node_token` 作为路径参数并以 `type=***REDACTED***
