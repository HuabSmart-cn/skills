...
function significantTokens(value) {
  return normalize(value).split(" ").filter((token) =***REDACTED***
}

function acronym(value) {
  const tokens = ***REDACTED***
  return tokens.length >= ***REDACTED***
}
...
function tokensPrefixCompatible(query, candidate) {
  const queryTokens = ***REDACTED***
  const candidateTokens = ***REDACTED***
  if (queryTokens.length < 2 || queryTokens.length !=***REDACTED***
  return queryTokens.every((token, index) =***REDACTED***
