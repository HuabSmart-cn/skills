...
| 文档 URL 或 doc_id，文档中已有画板 | `lark-cli docs +fetch --doc <URL> --as user`，从返回的 `<whiteboard token=***REDACTED***
...
具体命令示例、`--overwrite`、`--idempotent-token` 和 `--as user` 的使用方式，统一参考 [`whiteboard +update`](./lark-whiteboard-update.md)。
