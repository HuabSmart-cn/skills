...
- Do not open a browser or run a separate validation pass by default. Validate when the user explicitly asks or the Maps guidance below requires it. The bundled `python3 scripts/render.py <absolute-fragment-path> [<destination>.html] [--serve]` can wrap a fragment as standalone HTML or temporarily serve it for requested or required browser inspection. The rendered preview places the fragment inside a sandboxed iframe: scope Playwright locators to `page.frameLocator("iframe")` and evaluation to that frame.
...
- Prefer `--viz-series-1` through `--viz-series-6` for chart series; use `--foreground` and `--border` for neutrals, cycle the six series tokens when more are needed, and never use literal or fallback colors. Give every SVG label `fill: ***REDACTED***
