...
1. **缺 Key / 鉴权失败**：未配置 `LINKFOX_AGENT_API_KEY`，或任何 linkfox-* skill 返回 `errcode=***REDACTED***
...
- `error` 含 `团队不存在` 或 `getApiToken/generateApiToken` → 引导用户前往 https:***REDACTED***
...
```bash
echo 'export LINKFOX_AGENT_API_KEY=***REDACTED***
# 或
echo 'export LINKFOX_AGENT_API_KEY=***REDACTED***
