...
def _login_by_token(access_token: ***REDACTED***
    ...
    print(f"{_stderr_tag()} loginByToken 成功，新用户积分已触发发放", file=***REDACTED***
    return {}


def _fetch_user_info(access_token: ***REDACTED***
...
def _get_or_generate_api_token(access_token: ***REDACTED***
    ...
    # 成功响应可能直接是 token 字符串，或 {token:***REDACTED***
    token = ***REDACTED***
    if token:
        ***REDACTED***
...
def _extract_token(resp: ***REDACTED***
    """从响应里提取 token 字符串。
