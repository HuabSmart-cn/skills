...
def _debug(msg):
    if os.environ.get("TDOC_DEBUG") == "1" or os.environ.get("WORKBUDDY_TDOC_DEBUG") == "1":
        print(f"[TencentDocsPluginToken] {msg}", file=***REDACTED***
...
def _load_tokens():
    ***REDACTED***
    ...
    token_url = ***REDACTED***
    token_url += ***REDACTED***
    _debug(f"request connector-proxy token provider: ***REDACTED***
    try:
        ...
        _debug(f"connector-proxy token provider request failed: ***REDACTED***
