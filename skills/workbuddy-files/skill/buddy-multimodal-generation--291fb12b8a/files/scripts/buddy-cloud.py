...
def _sign_request(
    ...
    secret_date = ***REDACTED***
        (_d("VEMz") + secret_key).encode("utf-8"), date.encode("utf-8")
    )
    secret_service = ***REDACTED***
    secret_signing = ***REDACTED***
...
_ACTIVE_TOKEN = ***REDACTED***


def _redact_token(text: ***REDACTED***
...
def main():
    ...
    # Resolve token with security priority: ***REDACTED***
    token = ***REDACTED***
    ...
    if not token:
        ***REDACTED***
            ...
            "message": (
                "Authentication token is required. Use one of (in order of security):***REDACTED***
...
... [Content truncated, total 29,439 chars] ...