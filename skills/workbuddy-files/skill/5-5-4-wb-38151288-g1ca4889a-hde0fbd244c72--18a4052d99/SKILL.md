...
| **Livestream poster** | livestream poster, live stream poster, webinar poster · 直播海报/直播宣传图/直播预告海报/线上直播海报 | `ardot-design-core` + `livestream-poster` |
| **Poster / visual** | poster, flyer, banner, billboard, cover, logo, icon, illustration, brand/VI · 海报/宣传单/横幅/banner/封面/图标/插画/Logo/品牌 | `ardot-design-core` + `ardot-poster` |
...
2. Once classified (non-unclear), **immediately** load that skill via `Skill(<name>)`, then follow its workflow.
3. If the user select an existing node and ask for a local edit → still `ardot-ui-design` (compositional path), unless the node is clearly a slide/poster.
