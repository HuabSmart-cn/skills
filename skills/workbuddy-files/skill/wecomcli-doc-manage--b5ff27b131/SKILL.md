...
- **`keywords` 必须先分词再组装**：当用户给出自然语言 query（如 `"帮我找下产品的待办tool文档"`）时，禁止把整段 query 直接当成单个 keyword 传入。处理流程：
  ...
  3. 组装 `keywords` 数组：第 1 个元素是所有"必传 token"用空格拼接的串（只拼必传的，不要把全部 token 都塞进去），后续元素依次是各单独 token（必传 + 辅助）。例如 query `"帮我找下产品的待办tool文档"`，分词后必传 token 为 `["待办", "tool"]`，则 `keywords = ***REDACTED***
  4. 若必传 token 只有 1 个，第 1 个元素就是该 token 本身，不必重复追加。例如 query `"周报"` → `keywords = ***REDACTED***
