...
Usage:
    register_expert.py <expert-dir> [--marketplace-dir <marketplace-dir>]

Examples:
    python3 register_expert.py <expert-dir>
    python3 register_expert.py ./my-expert --marketplace-dir <marketplace-dir>

The creating session id is read from CODEBUDDY_SESSION_ID when present.
"""
...
SESSION_ID_ENV_VAR = 'CODEBUDDY_SESSION_ID'
UNEXPANDED_SESSION_ID_ARGS = {'$CODEBUDDY_SESSION_ID', '${CODEBUDDY_SESSION_ID}'}
...
def normalize_session_id(value: Optional[str]) -> Optional[str]:
    """Return a usable session id, ignoring empty or unexpanded shell placeholders."""
    session_id = (value or '').strip()
    if not session_id or session_id in UNEXPANDED_SESSION_ID_ARGS:
        return None
    return session_id


def get_session_id(explicit_session_id: Optional[str]) -> Optional[str]:
    """Resolve the creating session id from CLI arg or Bash runtime env."""
    return normalize_session_id(explicit_session_id) or normalize_session_id(os.environ.get(SESSION_ID_ENV_VAR))
...
def main():
    ...
    session_id = get_session_id(session_id_arg)

    # Register
    success = register_expert(expert_dir, marketplace_dir, session_id)
    if success and session_id:
        print(f"\n🎉 Expert '{expert_dir.name}' is now registered and visible in WorkBuddy!")
    elif success:
        print(f"\n✅ Expert '{expert_dir.name}' is registered in marketplace.json.")
        print("Warning: no session id found; it may not appear in My Experts for the current user automatically.")
    else:
        print(f"\n❌ Registration failed")
        sys.exit(1)
