...
| `next_page_token` | string | 下一页游标（`has_more=***REDACTED***
...
通过 `--short-summary` 标志获取滚动纪要，**必须提供 `--minute-id`**（稳态纪要唯一标识）。命令会自动循环分页拉取（使用返回的 `next_page_token` 不断请求下一页，直到 `has_more=***REDACTED***
