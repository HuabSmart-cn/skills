...
1. 查询板块行情榜：westock sector ranking
   → 输出行业/概念板块排行（默认涨幅降序；--type 可切换主力净流入等字段、--order asc 升序）
...
> 3. **限量采样**：开放筛选出池时用小 `--limit`（如 20~30），**基于 Top N 代

... [Content truncated, total 27,223 chars] ...