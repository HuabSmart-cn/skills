...
业务代码通过 **`keychain.GetAccessToken(account)` / `keychain.SetAccessToken(account, token)`** 统一读写，不直接拼环境变量或 keyring。**`SetAccessToken`** 返回 `(storedInKeyring bool, err)`：`false` 且 `err=***REDACTED***
