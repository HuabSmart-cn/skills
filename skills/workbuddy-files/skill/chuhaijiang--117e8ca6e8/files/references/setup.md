...
- **API Key（兼容）**（二选一，服务端优先级 header > query）：
  - HTTP 请求头 `X-API-Key: ***REDACTED***
  - URL 查询参数 `?api_key=***REDACTED***
...
按该客户端官方文档的远程 MCP（streamable HTTP）配置格式写入 OAuth 端点并尝试 OAuth；不支持 OAuth 时改用 API Key 端点，支持自定义请求头就用 `X-API-Key`，否则把 `?api_key=***REDACTED***
