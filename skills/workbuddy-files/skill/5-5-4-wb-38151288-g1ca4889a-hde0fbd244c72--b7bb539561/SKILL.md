...
description_en: Use WorkBuddy's native Library to create or organize online documents, manage database records, import CSV or Excel data, build dashboards and report pages, publish Markdown as web pages or presentation HTML, manage folders and drive files, review revisions, and share content. Use for Library, knowledge base, drive, Space, or workbuddy.ai/space links unless the user explicitly names another document product.
version: 0.5.16
author: csig-x2
level: personal
metadata:
...
说「建文件夹/做目录/把文档归到一起/建知识库」时，不默认建空间；默认用真·文件夹（纯容器目录节点）承接，措辞用「挂到下面/目录树」，不用「父/子节点」：

| 做法 | 怎么做 | 适合 |
|---|---|---|
| 文件夹（默认） | `manage/create_folder.py` 建纯容器目录节点，其余文档用 `move-node` 挂到它下面成目录树 | 纯归类整理 |
| 文档目录页 | 新建一篇文档当目录页（可在页内写导航/索引正文），其余文档挂到它下面 | 目录本身要写说明/导航时 |
| 独立空间 | 单独建空间统一管理；长期+多人协同推荐团队空间 | 长期沉淀 |

默认走「文件夹」；用户明确要「目录页里写点说明/索引」时走文档目录页；语义偏长期沉淀或多人协同时走独立空间。回执给对应链接（空间 `/space/s/{spaceId}`、节点 `/space/d/{nodeBlockId}`）；落地命令见 `manage/entry.md`。
...
| 建文件夹 / 新建目录 / 把文档归到一起 / 整理归类乱掉的目录树 | `manage/entry.md` |
...
**响应约定**：成功 → stdout 输出结构化文本（`KS_*` 前缀行 / 单行 JSON / XML 块）；失败 → stdout 仅一行 `{"error":"<脱敏错误>"}` 后 `exit 0`（含 header `traceid` 时附带）。后端 / HTTP 失败的错误内容为 `code=<错误码>; msg=<安全业务说明>`；无后端码的本地参数校验可保留明确文本。脚本只读取后端信封的 `code/msg`；`msg` 经截断和脱敏后透传，不透传其它响应字段、requestId、请求体、Token、Cookie、堆栈、内部路径或完整签名 URL。SKILL 层据「stdout 是 `KS_*` 还是 `{"error":...}`」判定走堆。**若 stdout 含 `KS_USER_REPLY\t...`，最终用户回执必须直接原样透传该行正文，禁止自行拼接链接、改写为近似文案或补写内部 ID。**
