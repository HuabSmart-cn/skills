    ...
    ***REDACTED***
    ...
    try:
        ...
        try:
            ...
            print("TOKEN_EXPIRED", file=***REDACTED***
            return None

    except FileNotFoundError:
        print("TOKEN_MISSING", file=***REDACTED***
        return None
...
def _save_token_file(token: ***REDACTED***
    """将 token 和时间戳写入缓存文件（权限 600）"""
    TOKEN_FILE.parent.mkdir(parents=***REDACTED***
    data = {
        "token": ***REDACTED***
...
def main():
    ...
    parser.add_argument("--token", "-t", default=***REDACTED***
    parser.add_argument("--data-type", "-d", default="all", choices=["all", "api", "doc"], help="数据类型 (默认: all)")
    parser.add_argument("--save-token", default=***REDACTED***
