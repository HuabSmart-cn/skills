...
| 环境变量 | 含义 | 透传 header |
|---|---|---|
| `TDOC_ONEID_ACCESS_TOKEN` | SaaS 端 OneID access token（推荐） | `X-Oneid-Access-Token: ***REDACTED***
| `TDOC_OAUTH_ACCESS_TOKEN` | C 端 OAuth access token（双票场景可同时存在） | `Authorization: ***REDACTED***
