...
只询问尚未提供、且无法从上下文可靠推断的信息。用 `AskUserQuestion` 收集仍缺失的项（人物与 Logo **分两题**，选项文案与 [style-selection.md](style-selection.md) Step 1 一致）。

⛔ **本步必须在 `create_design` / ImageGen 之前完成**。人物照片三选项必须包含 `我稍后上传`；prompt 中的讲师/嘉宾姓名不等于已上传照片。任一字段选择「我稍后上传」或「我稍后提供完整文案」时，停止当前轮并等待补齐。
