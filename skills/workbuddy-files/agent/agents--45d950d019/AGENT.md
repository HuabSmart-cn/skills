...
**串联：提 B 的风格 → design-token → doc-typeset(仿写) → html-review（不通过则定向改一次）→ 输出 html。** B 全程只读、只提风格（`reason: ***REDACTED***
...
② design-token skill   ← 传 { genre, user_query, ref_style_features } → design_tokens（融合 B 的风格）
   ↓
③ doc-typeset skill    ← 传 { 内容, design_tokens, genre, ref_html, mode=***REDACTED***
                          （保留 B 的视觉风格，用新内容重组结构）
   ↓
