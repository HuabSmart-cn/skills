...
However, it is also possible to use the already configured template within the tokenizer by specifying chat_template: ***REDACTED***
...
The train_on_eos config means that it would mask all EOS tokens for turns that aren’t assistant-turns. The other options are: ***REDACTED***
...
Please make sure that your tokenizer.eos_token is same as EOS (End-of-Sequence) token in template. Otherwise, set eos_token under special_tokens:***REDACTED***
...
You can add those tokens as new tokens under tokens: ***REDACTED***

If EOS token only appears at the end of a prompt, train_on_eos: ***REDACTED***
...
Each prompt must be have a key called segm

... [Content truncated, total 45,917 chars] ...