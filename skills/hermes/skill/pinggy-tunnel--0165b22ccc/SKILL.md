...
- **Pro mode requires the token as the username, not a flag.** Use `"$PINGGY_TOKEN+a.pinggy.io"` (no `free@`). With a token you can also add `:***REDACTED***
...
The remote client connects to `$URL` with `Authorization: ***REDACTED***
...
# Verify
curl -s "$URL/api/tags" -H "Authorization: ***REDACTED***
```

`co` enables CORS so a browser caller can hit the endpoint. Drop `co` for backend-only callers. For an OpenAI-compatible vLLM/llama.cpp endpoint, callers use base URL `$URL/v1` with `Authorization: ***REDACTED***
...
echo "Dev server password: ***REDACTED***
