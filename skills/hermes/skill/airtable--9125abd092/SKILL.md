...
1. Create a **Personal Access Token (PAT)** at https:***REDACTED***
...
4. Store the token in `${HERMES_HOME:***REDACTED***
   ```
...
2. **Find the base.** List bases (step above) OR ask the user for the `app...` ID directly if the token lacks `schema.bases:***REDACTED***
...
- **Per-base token scoping.** A `403` on one base while another works means the token's Access list doesn't include that base — not a scope or auth issue. Send the user to https:***REDACTED***
...
- **`AIRTABLE_API_KEY` flows from `${HERMES_HOME:***REDACTED***
