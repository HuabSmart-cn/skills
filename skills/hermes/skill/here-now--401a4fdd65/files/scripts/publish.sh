...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --slug)         SLUG="$2"; shift 2 ;;
    --claim-token)  CLAIM_TOKEN=***REDACTED***
...
if [[ -z "$API_KEY" && -f "$CREDENTIALS_FILE" ]]; then
  API_KEY=***REDACTED***
  [[ -n "$API_KEY" ]] && API_KEY_SOURCE=***REDACTED***
fi
...
if [[ -n "$API_KEY" && "$BASE_URL" != ***REDACTED***
...
if [[ -n "$FROM_DRIVE" ]]; then
  ...
  echo "publish_result.api_key_source=***REDACTED***
