...
def refresh_token(token_data: ***REDACTED***
    ...
    token_data["token"] = ***REDACTED***
    token_data["expiry"] = ***REDACTED***
        datetime.now(timezone.utc).timestamp() + result["expires_in"],
        tz=timezone.utc,
    ).isoformat()

    get_token_path().write_text(
        json.dumps(_normalize_authorized_user_payload(token_data), indent=***REDACTED***
    )
    return token_data
...
def main():
    ...
    env = os.environ.copy()
    env["GOOGLE_WORKSPACE_CLI_TOKEN"] = ***REDACTED***
