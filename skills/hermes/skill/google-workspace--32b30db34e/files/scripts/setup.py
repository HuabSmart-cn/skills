...
def _load_token_payload(path: ***REDACTED***
    try:
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        "Token is valid but missing required Google Workspace scopes:***REDACTED***
        f"{bullets}\n"
...
def check_auth(quiet: bool = False):
    ...
    try:
        ...
        print(f"TOKEN_CORRUPT: ***REDACTED***
        return False
    ...
    print("TOKEN_INVALID: ***REDACTED***
    return False


def store_client_secret(path: ***REDACTED***
    ...
    CLIENT_SECRET_PATH.write_text(json.dumps(data, indent=***REDACTED***
...
def exchange_auth_code(code: str):
    ...
    try:
        ...
        os.environ["OAUTHLIB_RELAX_TOKEN_SCOPE"] = ***REDACTED***
        flow.fetch_token(code=***REDACTED***
    except Exception as e:
    ...
    TOKEN_PATH.write_text(json.dumps(token_payload, indent=***REDACTED***
    PENDING_AUTH_PATH.unlink(missing_ok=True)
    print(f"OK: Authenticated. Token saved to {TOKEN_PATH}")
    print(f"Profile-scoped token location: ***REDACTED***
...
def revoke():
    ...
    try:
        ...
        print(f"Remote revocation failed (token may already be invalid): ***REDACTED***

    TOKEN_PATH.unlink(missing_ok=***REDACTED***
    PENDING_AUTH_PATH.unlink(missing_ok=True)
...
def main():
    ...
    group.add_argument("--client-secret", metavar=***REDACTED***
