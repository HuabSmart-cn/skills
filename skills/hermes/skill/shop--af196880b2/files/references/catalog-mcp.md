...
Signing in is **not required** — unauthenticated calls (profile only, no `Authorization`) still work. When you have an `access_token` (see device authorization in [direct-api.md](direct-api.md)), exchange it for a catalog token and send that as `Authorization: ***REDACTED***
...
The returned `access_token` is the catalog token. Keep it in memory only and add `Authorization: ***REDACTED***
