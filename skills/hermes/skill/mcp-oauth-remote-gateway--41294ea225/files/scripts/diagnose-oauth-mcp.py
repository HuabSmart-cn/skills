...
def main():
    ...
    print(f"[1] stored-token initialize -> HTTP {status} ok=***REDACTED***
    if ok:
    ...
    print(f"[2] token_endpoint=***REDACTED***

    rstatus, _rh, rbody = _post(
        token_ep, form=***REDACTED***
    ...
    print(f"[2] refresh -> HTTP 200 new_token_len=***REDACTED***
    ...
    print(f"[2b] new-token initialize -> HTTP {status2} ok=***REDACTED***
    if ok2:
        if args.write:
            new = dict(tok)
            new.update({"access_token": ***REDACTED***
