...
Cola exposes `gws` on `PATH` at startup (a wrapper over this skill's bundled binary). Run the bare command `gws`. Do not search `scripts/bin`, and do not use another `gws` from the user's machine.

If `gws` is missing, report that Google Calendar is not ready yet — never describe it as an account problem or a broken connector. This skill ships on macOS and Windows only.
...
如果用户指定的 Google 日历尚未连接或授权已经失效，完成 Google 授权就是执行原请求的必要前置步骤。

使用上下文中已有的 Google 账号、日历和授权信息，不重复询问已经明确的信息。

开始授权前，先简要列出完整的授权流程，让用户了解后续步骤；再根据当前进度，详细引导用户完成当前一步。

如果同时有官方操作文档和可直接打开的官方授权入口，两者都提供给用户，并主动打开授权入口，方便用户快速完成登录。
...
This installation is **calendar-only**. Authorization covers Google Calendar and nothing else: ***REDACTED***
...
The printed URL is already fully encoded (`+` in the query string is a space) — pass it unchanged, protected by shell quotes. Re-encoding it corrupts it: `+` → `%2B` makes Google reject the request with `400 invalid_request`. Percent-encode only URLs you construct yourself from raw values.

3. If login says no OAuth client is configured, Cola did not provision its Google client. Report that Google Calendar is not ready in this Cola build. Do not run `gws auth setup`, do not invent a client id, and do not ask the user for one.

4. If an API call fails with 403 mentioning `serviceusage`, `x-goog-user-project`, or "permission to use project": Cola's build passed a quota project it should not have. This is a Cola configuration bug — not the user's account, and not something a Google setting fixes. Report it as such and suggest updating Cola. Never direct the user to Google Cloud Console, never ask them to configure a project, and never edit the stored client configuration.

5. An auth command that runs far past its usual few seconds with no output is usually waiting on something, not dead. One known precondition: on macOS, the first credential access (or the first after an app update) can raise a system Keychain permission dialog that a background process cannot bring to the front. So do not react blindly — no killing, no second attempt — until you know what it is waiting for; check the platform and situation, then guide the user accordingly. On macOS, ask them to look for a system dialog saying gws wants to access "gws-cli" in the keychain (it may be hidden behind other windows) and approve it with "Always Allow" (their Mac login password may be required). The command resumes once approved; continue the flow and confirm with `gws auth status`.

6. Do not start a second login while one is waiting for the browser.
...
This build reads `https_proxy` / `HTTPS_PROXY`, `http_proxy` / `HTTP_PROXY`, and `all_proxy` / `ALL_PROXY`. Token exchange and Calendar API calls are HTTPS (`oauth2.googleapis.com`, `www.googleapis.com`). Prefer HTTP CONNECT (`https_proxy=***REDACTED***
...
A failed agenda or event-list command means the result is unknown. Never report it as zero events or an empty calendar.
