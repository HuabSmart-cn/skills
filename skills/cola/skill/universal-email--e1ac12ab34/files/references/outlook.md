...
Do not copy a password, app password, client ID, client secret, or token into chat. Do not send Microsoft app-password or IMAP setup help; this path does not collect a code.

## Locate the helper

Use the `cola-outlook-mail-auth` command Cola put on `PATH`. If the command is missing, tell the user Outlook mail is not ready yet — never ask them to install or run a client. When writing Himalaya `token.command`, store the absolute path from `command -v cola-outlook-mail-auth` (Windows: ***REDACTED***

## 对用户表达的要点与示例

保留下列事实、安全提醒和下一步；措辞可结合上下文自然调整。
...
You run every step. Use the communication guidance above. Do not ask them to run a command.
...
msgraph.auth.token.command = ***REDACTED***
```
...
If an HTTP or HTTPS proxy is already on the device, apply it to the **first** `connect` (and to `doctor` / `token`) — do not wait for a timeout. Prefer HTTP CONNECT (`https_proxy=***REDACTED***
