...
| Provider | Ordered wraps; skip unavailable entries and remove duplicates |
| --- | --- |
| Gmail IMAP/SMTP | HTTP CONNECT if declared → SOCKS if declared → direct. With no declared proxy, direct is first. |
| QQ Mail | Direct → HTTP CONNECT if declared → SOCKS if declared. Do not change QQ credentials between wraps. |
| NetEase Mail (163 / 126 / yeah) | Direct → HTTP CONNECT if declared → SOCKS if declared. Do not change the 客户端授权码 between wraps. |
| iCloud Mail | Direct → HTTP CONNECT if declared → SOCKS if declared. |
| Outlook through Microsoft Graph | HTTP CONNECT if declared → SOCKS if declared → direct. Do not reuse IMAP/SMTP conclusions. |
| Other IMAP/SMTP | The device's normal route → the other declared route → direct. Verify what “normal” selected before continuing. |
...
This is a recovery guide, not a required preflight. First run the user's requested operation with the normal bare Himalaya command and a 25-second tool timeout. If it succeeds, stop: do not inspect proxy settings or replace a working route. Read and apply the rest of this guide only after a network-stage failure or when the user explicitly requires proxy routing or diagnosis.

After recovery starts, build the provider's ordered list above, skip routes that are not declared, remove duplicates, and skip the route already tested when it can be identified. Run one Himalaya command at a time. Stop at the first wrap that completes the current command. Reuse that wrap for the rest of the session. Do not re-probe on every later mail operation. Do not ask the user to toggle Cola's proxy.

For an existing account, keep retrying the user's requested operation rather than substituting a broad validation command. A read request must not start with combined `account check`: combined validation also waits for SMTP, so a blocked SMTP route can turn a working inbox into a long apparent failure. Each attempt ends at 25 seconds; do not leave the user waiting on the same route for minutes.

Use validation after creating or repairing an account:

```bash
himalaya --account <name> --log-level debug --json account check
```

on the **first wrap**. Tool timeout 25s. If it succeeds, done. For diagnosis of a read-only task, prefer the requested IMAP operation or `--backend imap`; do not test SMTP without a reason.
...
Once explicit recovery starts, apply the provider table literally. Do not use another bare retry in a mixed proxy environment: because `all_proxy` wins, “bare” is an implicit SOCKS choice rather than a controlled test of the intended route.

Direct must unset **both** letter cases. Cola writes `ALL_PROXY` / `HTTPS_PROXY`; unsetting only `all_proxy` is not a direct test:
...
SOCKS wrap when HTTP variables must not interfere (replace the URL with the discovered `socks5://HOST:PORT`):

```bash
env -u https_proxy -u HTTPS_PROXY -u http_proxy -u HTTP_PROXY \
  all_proxy='socks5://HOST:PORT' ALL_PROXY='socks5://HOST:PORT' \
  himalaya --account <name> --log-level debug --json account check
```
...
| QQ Mail, NetEase Mail, or iCloud Mail becomes unreachable only when proxied | A global proxy is interfering with a provider that normally uses the local route. | Direct wrap for that invocation. Do not disable the user's global proxy or change credentials. |
...
Tell the user the matching outcome from the provider guide in natural language (Gmail: `gmail.md`).
