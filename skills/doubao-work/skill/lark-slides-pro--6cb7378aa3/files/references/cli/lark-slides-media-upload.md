...
把本地图片上传到指定演示文稿的 drive 媒体库，返回 `file_token`。**返回的 token 作为 `<img src=***REDACTED***
...
# 传 wiki URL（CLI 自动 wiki.spaces.get_node 解析为真实 token，校验 obj_type=***REDACTED***
lark-cli slides +media-upload \
  --file ./pic.png \
...
- **`file_token`**：把它写进 `<img src=***REDACTED***
