...
| file / multi-file | 先 `meegle-cli attachment +upload --resource-type 15 --project-key <K> --work-item-type <type> --field-key <field_key> <local-path>` 拿 `file_token`（工作项尚未创建，传 `--work-item-type` 而非 `--work-item-id`），再 **stringify** 数组 `"[{\"name\":***REDACTED***
