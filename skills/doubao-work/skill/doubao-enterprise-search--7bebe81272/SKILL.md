...
- `enterprise_agentic_search` 返回图片时，仅保留承载关键信息或能显著帮助理解的相关图片；需要展示时，必须直接使用 Markdown 图片语法 `![{图注}]({图片URL})` 输出，不得只输出文字摘要或图片标题，不得调用 `present_files` 交付，也不得使用普通 Markdown 链接或裸 URL 代替；每张图片只输出一次。
