...
完整回读示例：`lark-cli docs +fetch --api-version v2 --doc "<doc-url-or-token>" --scope full`。目录或 block ID 的简要读取示例：`lark-cli docs +fetch --api-version v2 --doc "<doc-url-or-token>" --scope outline 2>&1 | head -80`，这是合法命令。若返回内容实际出现 `validation: ***REDACTED***
